...
const char *sensorHealthStatus(const SensorDiagnostic &d) {
  if (!runtimeControlReady || isCenter) return "inactive";
...
bool canSendFrame(uint32_t actuatorID, const byte *data, byte dataLen) {
  if (isCenter || !canInitialized) return false;
...
void handleHyperspawnRxFrame(uint32_t id, const byte *data, byte len) {
  if (operatingMode != OPERATING_HYPERSPAWN_ROUTE || isCenter) return;
...
void sendHyperspawnState() {
  if (!canInitialized || isCenter || operatingMode != OPERATING_HYPERSPAWN_ROUTE) return;
...
void hyperspawnRouteTask(void *parameter) {
  ...
  while (true) {
    ...
    if (runtimeControlReady && operatingMode == OPERATING_HYPERSPAWN_ROUTE && !isCenter) {
      const uint32_t now = millis();
      expireHyperspawnFragments(now);
...
void printStatus() {
  dbPrintf("Operating: %s | Mode: %s | chirality: %s | play:%d | raw:%d | maxTorque:%.2f A\n",
                operatingModeName().c_str(),
...
const char *canHealthStatus() {
  if (!configProvisioned || isCenter) return "inactive";
...
String buildDiagnosticsJson() {
  ...
  out.reserve(18000);
  const uint32_t now = millis();
  const bool legRuntime = configProvisioned && !isCenter && runtimeControlReady;
  ...
  out += "\"spi\":{";
  out += "\"status\":\"" + String((!configProvisioned || isCenter) ? "inactive" : (spiInitialized ? "ok" : "fault")) + "\",";
  ...
  out += "\"encoder_io\":{";
  out += "\"status\":\"" + String((!configProvisioned || isCenter) ? "inactive" : (runtimeControlReady ? "ok" : "fault")) + "\",";
  ...
  out += "\"i2c\":{";
  out += "\"status\":\"" + String(i2cInitialized ? "ok" : "fault") + "\",";
...
void handleApiConfigPost() {
  ...
  if (server.hasArg("operatingMode")) {
    ...
    if (requested != operatingMode) {
      if (runtimeControlReady && !isCenter) requestStop(3);
      clearAllTorqueSetpoints();
      hyperspawnControlMode = HS_CONTROL_NONE;
      operatingMode = requested;
      rebootRequired = true;
...
<div class="shell">
 <div class="top">
  ...
  <div class="badges">
   <span class="badge" id="diagBadge">DIAG</span><span class="badge" id="playBadge">STATE</span><span class="badge" id="canBadge">ROLE</span><span class="badge" id="clientBadge">0 CLIENTS</span><span class="badge" id="heapBadge">HEAP</span>
  </div>
 </div>
 ...
 <section id="config" class="view">
  <div class="grid">
   ...
   </div><div class="tiny" style="margin-top:8px">Role controls LEFTLEG/RIGHTLEG/CENTER AP identity. Operating path selects local portal/Serial authority or the parallel HyperSpawn ROS2 CAN route. Either topology change requires reboot.</div></div>
...
async function loadState(){
 try{
  ...
  document.getElementById('roleTitle').textContent=(state.role||'unconfigured').toUpperCase();document.getElementById('operatingText').textContent=(state.operating_mode||'standalone').toUpperCase();
  document.getElementById('ipText').textContent='@ '+state.ip+' · '+state.ssid+' · AS5600 GPIO '+(state.sensor_pins||[]).join('/');
...
function sendTorque(j){if(!state||!['left','right'].includes(state.role))return notice('Configure a leg role first.');if(state.operating_mode==='hyperspawn')return notice('Manual torque is locked: HyperSpawn/ROS2 owns command authority. STOP remains available.');cmd(`torque ${state.role} ${j} ${document.getElementById('t_'+j).value}`)}
function sendImpedance(j){if(!state||!['left','right'].includes(state.role))return notice('Configure a leg role first.');if(state.operating_mode==='hyperspawn')return notice('Manual impedance is locked: HyperSpawn/ROS2 owns command authority.');const on=document.getElementById('ie_'+j).checked?1:0;cmd(`impedance ${state.role} ${j} ${on} ${document.getElementById('ip_'+j).value} ${document.getElementById('iv_'+j).value}`)}
...
async function loadConfig(){
 try{
  config=await jfetch('/api/config');document.getElementById('cfgRole').value=['left','right','center'].includes(config.role)?config.role:'left';document.getElementById('cfgMaxTorque').value=config.max_torque;document.getElementById('cfgOperatingMode').value=config.operating_mode||'standalone';document.getElementById('cfgHsTimeout').value=config.hyperspawn_timeout_ms||250;document.getElementById('cfgHsScale').value=config.hyperspawn_position_units_per_degree||1;document.getElementById('cfgHsLegacy').checked=!!config.hyperspawn_legacy_broadcast;document.getElementById('cfgHsAutoArm').checked=!!config.hyperspawn_auto_arm;
...
function configParams(){
 const p=new URLSearchParams({role:document.getElementById('cfgRole').value,operatingMode:document.getElementById('cfgOperatingMode').value,max_torque:document.getElementById('cfgMaxTorque').value,hsTimeout:document.getElementById('cfgHsTimeout').value,hsScale:document.getElementById('cfgHsScale').value,hsLegacy:document.getElementById('cfgHsLegacy').checked?'1':'0',hsAutoArm:document.getElementById('cfgHsAutoArm').checked?'1':'0'});
...
void setup() {
  ...
  dbPrintf("Dropbear controller ready: role=%s operating=%s portal=%s SSID=%s. Type 'help'.\n",
           selectedRoleName().c_str(), operatingModeName().c_str(),
